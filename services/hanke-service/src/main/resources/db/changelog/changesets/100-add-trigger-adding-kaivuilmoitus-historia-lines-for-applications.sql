...
--changeset Petri Timlin:100-add-trigger-adding-kaivuilmoitus-historia-lines-for-applications
--comment: Add trigger for checking changes of kaivuilmoitus and inserting rows to kaivuilmoitus_historia

CREATE FUNCTION check_kaivuilmoitus_changes()
    RETURNS TRIGGER
    LANGUAGE PLPGSQL
AS '
  DECLARE
  ...
  BEGIN
    IF (TG_OP = ''INSERT'' AND NEW.applicationtype=''EXCAVATION_NOTIFICATION'') THEN
        ...
        VALUES (
            ...
            (select date_reported from valmistumisilmoitus where application_id=NEW.id and type=''TYO_VALMIS''),
    ...
    ELSIF (TG_OP = ''UPDATE'' AND NEW.applicationtype=''EXCAVATION_NOTIFICATION'') THEN
        ...
        select date_reported into toiminnallisen_kunnon_ilmoitettu_paivaNew from valmistumisilmoitus where application_id=NEW.id and type=''TOIMINNALLINEN_KUNTO'';
        select date_reported into tyo_valmis_ilmoitettu_paivaNew from valmistumisilmoitus where application_id=NEW.id and type=''TYO_VALMIS'';
...
CREATE CONSTRAINT TRIGGER after_kaivuilmoitus_changes
    AFTER INSERT OR UPDATE OR DELETE
    ON applications
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW
EXECUTE FUNCTION check_kaivuilmoitus_changes();