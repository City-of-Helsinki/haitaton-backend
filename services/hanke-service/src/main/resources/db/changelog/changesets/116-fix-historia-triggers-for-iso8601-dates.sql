...
CREATE OR REPLACE FUNCTION check_johtoselvitys_changes()
    RETURNS TRIGGER
    LANGUAGE PLPGSQL
AS '
  DECLARE
  ...
  BEGIN
    IF (TG_OP = ''INSERT'' AND NEW.applicationtype=''CABLE_REPORT'') THEN
        ...
        VALUES (
            ...
            (NEW.applicationdata ->> ''startTime'')::timestamptz,
            (NEW.applicationdata ->> ''endTime'')::timestamptz,
    ...
    ELSIF (TG_OP = ''DELETE'' AND OLD.applicationtype=''CABLE_REPORT'') THEN
        ...
        VALUES (
            ...
            (OLD.applicationdata ->> ''startTime'')::timestamptz,
            (OLD.applicationdata ->> ''endTime'')::timestamptz,
