...
--changeset Teemu Hiltunen:115-add-status-to-hankealue
--comment: Add status column to hankealue and hankealue_historia, update trigger function

-- Add status column to hankealue table with default value PUBLIC
ALTER TABLE hankealue ADD COLUMN status VARCHAR(10) NOT NULL DEFAULT 'PUBLIC';

-- Add status column to hankealue_historia table
ALTER TABLE hankealue_historia ADD COLUMN status VARCHAR(10);

-- Drop the existing trigger and function
DROP TRIGGER IF EXISTS after_hankealue_changes ON hankealue;
DROP FUNCTION IF EXISTS check_hankealue_changes();
