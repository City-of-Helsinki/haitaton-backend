...
internal fun InvoicingCustomer.validateForErrors(path: String): ValidationResult =
    validate { notJustWhitespace(name, "$path.name") }
        .whenNotNull(registryKey) { validateTrue(it.isValidBusinessId(), "$path.registryKey") }
        .andWhen(!ovt.isNullOrBlank()) { validateTrue(ovt.isValidOVT(registryKey), "$path.ovt") }
...
internal fun InvoicingCustomer.validateForMissing(path: String): ValidationResult =
