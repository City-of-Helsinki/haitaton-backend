...
class HankeGeometriatValidator : ConstraintValidator<ValidHankeGeometriat, HankeGeometriat> {
    ...
    override fun isValid(value: HankeGeometriat?, context: ConstraintValidatorContext): Boolean {
        ...
        if (featureCollection.features.isNullOrEmpty()) {
            context.buildConstraintViolationWithTemplate(HankeError.HAI1011.name).addPropertyNode("featureCollection")
                .addPropertyNode("features").addConstraintViolation()
            ok = false
        }
        when {
            featureCollection.crs == null -> {
                context.buildConstraintViolationWithTemplate(HankeError.HAI1011.name)
                    .addPropertyNode("featureCollection").addPropertyNode("crs").addConstraintViolation()
                ok = false
            }
            featureCollection.crs.properties.isNullOrEmpty() -> {
                context.buildConstraintViolationWithTemplate(HankeError.HAI1011.name)
                    .addPropertyNode("featureCollection").addPropertyNode("crs").addPropertyNode("properties")
