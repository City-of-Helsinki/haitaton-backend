...
class RandomHankeGenerator(
    ...
    // Use the id of your test user (it can be checked from an existing hanke in its 'createdbyuserid' field)
    private val userId = "5f893af3-f7c0-433b-bf50-08f88fbc43a7"

    @Transactional(propagation = Propagation.REQUIRES_NEW)
    fun createRandomHankeWithRelations(index: Int) {
        val hanke = createRandomHanke(index)
    ...
    private fun createRandomHanke(index: Int): HankeEntity {
        val random = Random(index.toLong())
        
        val hankeNames = listOf(
            "Hämeentien perusparannus",
            "Mannerheimintien saneeraus", 
            "Kaisaniemenkadun päällystys",
            "Unioninkadun vesiputkiremontti",
            "Aleksanterinkadun katuvalot",
            "Bulevardi kaapeliasennukset",
            "Esplanadi lämmitysjärjestelmä",
            "Pohjoisesplanadi kaukolämpö",
            "Katajanokka vesijohto",
            "Kruununhaka sähkötyöt",
            "Punavuori tietyöt",
            "Kaartinkaupunki katualue",
            "Kamppi liikennejärjestelyt",
            "Töölö infrastruktuurihanke",
            "Kallio katutyöt"
        )
        
        val descriptions = listOf(
            "Kadun perusparannus ja uudet katuvalot",
            "Vesijohtojen uusinta ja tienpinnan parantaminen",
            "Kaukolämpöverkon laajentaminen",
            "Sähkökaapeleiden uusinta",
            "Liikennejärjestelyjen parantaminen",
            "Infrastruktuurin modernisoiminen",
            "Katuverkon saneeraus",
            "Viemärijärjestelmän uusinta"
        )
        
        val hanke = HankeEntity(
            status = HankeStatus.PUBLIC,
            hankeTunnus = hanketunnusService.newHanketunnus(),
            nimi = hankeNames[random.nextInt(hankeNames.size)] + " ${index + 1}",
            kuvaus = descriptions[random.nextInt(descriptions.size)],
        ...
        hanke.tyomaaKatuosoite = "Katu ${index + 1}, Helsinki"
        hanke.tyomaaTyyppi = mutableSetOf(TyomaaTyyppi.entries[random.nextInt(TyomaaTyyppi.entries.size)])

        return hanke
    }

    private fun createPerustaja(hanke: HankeEntity): HankekayttajaEntity {
        val permissionEntity = permissionService.create(
            hanke.id,
            userId,
            Kayttooikeustaso.KAIKKI_OIKEUDET,
        )
        val kayttaja = HankekayttajaEntity(
            hankeId = hanke.id,
            etunimi = "Pertti",
            sukunimi = "Perustaja",
    ...
    private fun createRandomHankealue(hanke: HankeEntity, index: Int): HankealueEntity {
        ...
        val geometriat = Geometriat(
            featureCollection = FeatureCollection().apply {
                add(Feature().apply {
                    setGeometry(geometry)
                    properties = mapOf(
                        "hankeTunnus" to hanke.hankeTunnus,
                        "nimi" to alueNames[random.nextInt(alueNames.size)]
                    )
                })
            },
            createdByUserId = userId,
        ...
        val hankealue = HankealueEntity(
            ...
            kaistaHaitta = VaikutusAutoliikenteenKaistamaariin.entries[random.nextInt(
                VaikutusAutoliikenteenKaistamaariin.entries.size)],
            kaistaPituusHaitta = AutoliikenteenKaistavaikutustenPituus.entries[random.nextInt(
                AutoliikenteenKaistavaikutustenPituus.entries.size)],
            ...
            nimi = alueNames[random.nextInt(alueNames.size)],
            tormaystarkasteluTulos = null,
            haittojenhallintasuunnitelma = DEFAULT_HHS.toMutableMap(),
        )
        
        val tormaystarkasteluTulos = tormaystarkasteluService.calculateTormaystarkastelu(hankealue)!!
        hankealue.tormaystarkasteluTulos = TormaystarkasteluTulosEntity(
    ...
    private fun createRandomGeometry(random: Random): Polygon {
        ...
        val helsinkiCenterX = 25496194.0
        val helsinkiCenterY = 6672000.0
        ...
        // Create a small rectangular area (10-200 meters)
        val width = random.nextDouble(10.0, 200.0)
        val height = random.nextDouble(10.0, 200.0)
    ...
    private fun createYhteyshenkilo(contact: HankeYhteystietoEntity, user: HankekayttajaEntity): HankeYhteyshenkiloEntity {
        return HankeYhteyshenkiloEntity(
            hankeKayttaja = user,
            hankeYhteystieto = contact,
        )
    }
