...
from modules.common import deploy, validate_data_count_limits


class GisProcessor:
    """Default implementation for validating and deploying."""
    ...
    def validate_deploy(self):
        ...
        if validate_result is True:
            deploy(self._pg_conn_uri, self._tormays_table_org, self._tormays_file_temp, self._logger)
        else:
            logging.error("Validation failed (%s). No deploy", self._module)
