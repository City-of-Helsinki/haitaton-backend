...
class HslBuses(GisProcessor):
    ...
    def _stoptimes_for_trip_ids(self, trip_ids: list[str]) -> pd.DataFrame:
        stp_ind = self.feed().stop_times["trip_id"].isin(trip_ids).values
        return self.feed().stop_times[stp_ind]

    def _summarize_rush_hours(self, stoptimes) -> pd.DataFrame:
        rush_hours = self._rush_hours()
        res = stoptimes.copy()
        for c_string, t_s, t_e in rush_hours:
            res[c_string] = stoptimes.apply(
                lambda r: datetime.strptime(r.arrival_time, "%H:%M:%S").time() >= t_s
                and datetime.strptime(r.arrival_time, "%H:%M:%S").time() < t_e,
                axis=1,
            )
        return res
    ...
    def _process_hsl_bus_lines(self) -> pd.DataFrame:
        ...
        sw_tmp1 = shapes_week_lines.merge(
        ...
        swa_tmp1 = self._add_trunk_descriptor(swa_tmp0)
